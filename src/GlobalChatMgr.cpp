...
{
    LOG_DEBUG("module", "WorldChat: Saving PlayerData for {}", player->GetName());
    ObjectGuid guid = player->GetGUID();
...
void GlobalChatMgr::SendGlobalChat(WorldSession* session, const char* message, TeamId toTeam)
{
    ...
    if (ProfanityBlockType > 0 && playerSecurity <= ProfanityBlockLevel && HasForbiddenPhrase(message))
    {
        ...
        if ((playerSecurity > 0 && ProfanityBlockType != 1) || ProfanityBlockType == 2)
        {
            ...
            LOG_INFO("module", "WorldChat: Player {} tried posting a forbidden message.", player->GetName());
            return;
        }
        ...
        if (ProfanityBlockType == 3)
        {
            if (ProfanityMute > 0)
            {
                ...
                LOG_INFO("module", "WorldChat: Player {} got muted for {} for posting a forbidden message.", player->GetName(), secsToTimeString(ProfanityMute));
    ...
    if (URLBlockType > 0 && playerSecurity <= URLBlockLevel && HasForbiddenURL(message))
    {
        ...
        if ((playerSecurity > 0 && URLBlockType != 1) || URLBlockType == 2)
        {
            ...
            LOG_INFO("module", "WorldChat: Player {} tried posting a forbidden URL.", player->GetName());
            return;
        }
        ...
        if (URLBlockType == 3)
        {
            if (URLMute > 0)
            {
                ...
                LOG_INFO("module", "WorldChat: Player {} got muted for {} for posting a forbidden URL.", player->GetName(), secsToTimeString(URLMute));
    ...
    LOG_INFO("module", "WorldChat: Player {}: {}", player->GetName(), chatText);
...
void GlobalChatMgr::PlayerJoinCommand(ChatHandler* handler)
{
    ...
    LOG_INFO("module", "WorldChat: Player {} joined WorldChat.", player->GetName());
}
...
void GlobalChatMgr::PlayerLeaveCommand(ChatHandler* handler)
{
    ...
    LOG_INFO("module", "WorldChat: Player {} left WorldChat.", player->GetName());
}
